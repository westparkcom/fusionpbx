...
//get posted data
	if (is_array($_POST['ibr_pilots'])) {
		$action = $_POST['action'];
		$search = $_POST['search'];
		$ibr_pilots = $_POST['ibr_pilots'];
	}

//process the http post data by action
	if ($action != '' && is_array($ibr_pilots) && @sizeof($ibr_pilots) != 0) {
		switch ($action) {
			case 'copy':
		...
		header('Location: ibr_pilots.php'.($search != '' ? '?search='.urlencode($search) : null));
		exit;
	}
...
//prepare to page the results
	$sql = "select count(*) from v_ibr_pilots ";
...
//get the list
	...
	$sql .= order_by($order_by, $order, 'ibr_pilot', 'asc');
	$sql .= limit_offset($rows_per_page, $offset);
	$database = new database;
	$ibr_pilots = $database->select($sql, $parameters, 'all');
...
//include the header
	$document['title'] = $text['title-ibr_pilots'];
	require_once "resources/header.php";
//show the content
	echo "<div class='action_bar' id='action_bar'>\n";
	echo "	<div class='heading'><b>".$text['title-ibr_pilots']." (".$num_rows.")</b></div>\n";
	...
	if (permission_exists('ibr_pilot_edit') && $ibr_pilots) {
	...
	if (permission_exists('ibr_pilot_delete') && $ibr_pilots) {
		echo button::create(['type'=>'button','label'=>$text['button-delete'],'icon'=>$_SESSION['theme']['button_icon_delete'],'name'=>'btn_delete','onclick'=>"modal_open('modal-delete','btn_delete');"]);
	}
	echo 		"<form id='form_search' class='inline' method='get'>\n";
	echo 		"<input type='text' class='txt list-search' name='search' id='search' value=\"".escape($search)."\" placeholder=\"".$text['label-search']."\" onkeydown='list_search_reset();'>";
	echo button::create(['label'=>$text['button-search'],'icon'=>$_SESSION['theme']['button_icon_search'],'type'=>'submit','id'=>'btn_search','style'=>($search != '' ? 'display: none;' : null)]);
	echo button::create(['label'=>$text['button-reset'],'icon'=>$_SESSION['theme']['button_icon_reset'],'type'=>'button','id'=>'btn_reset','link'=>'ibr_pilots.php','style'=>($search == '' ? 'display: none;' : null)]);
	...
	echo $text['title_description-ibr_pilot']."\n";
	echo "<br /><br />\n";
	...
	if (is_array($ibr_pilots) && @sizeof($ibr_pilots) != 0) {
		$x = 0;
		foreach ($ibr_pilots as $row) {
			...
			if (permission_exists('ibr_pilot_add') || permission_exists('ibr_pilot_edit') || permission_exists('ibr_pilot_delete')) {
				...
				echo "		<input type='checkbox' name='ibr_pilots[$x][checked]' id='checkbox_".$x."' value='true' onclick=\"if (!this.checked) { document.getElementById('checkbox_all').checked = false; }\">\n";
				echo "		<input type='hidden' name='ibr_pilots[$x][uuid]' value='".escape($row['ibr_pilot_uuid'])."' />\n";
				echo "	</td>\n";
			}
			echo "	<td>";
			if (permission_exists('ibr_pilot_edit')) {
				echo "<a href='".$list_row_url."' title=\"".$text['button-edit']."\">".escape($row['ibr_pilot'])."</a>";
			}
			else {
