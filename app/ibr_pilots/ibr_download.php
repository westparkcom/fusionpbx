...
//get the IBR pilots from the database and send them as output
	if (is_array($_REQUEST["column_group"]) && @sizeof($_REQUEST["column_group"]) != 0) {
		...
		if (is_array($selected_columns) && @sizeof($selected_columns) != 0) {
			$sql = "select ".implode(', ', $selected_columns)." from v_ibr_pilots ";
...
//begin the page content
	...
	echo "<form method='post' name='frm' action='ibr_download.php' autocomplete='off'>\n";

	echo "<div style='float: right;'>\n";
	echo "<input type='button' class='btn' alt='".$text['button-back']."' onclick=\"window.location='ibr_pilots.php'\" value='".$text['button-back']."'>\n";
