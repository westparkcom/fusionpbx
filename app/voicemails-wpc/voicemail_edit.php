			...
			if ($_POST["persistformvar"] != "true") {
				...
				//add voicemail escalations
					if (permission_exists('voicemail_escalation_add') && sizeof($voicemail_escalations) > 0) {
						foreach ($voicemail_escalations as $x => $voicemail_escalation) {
							if ($voicemail_escalation['voicemail_escalation_phonenum'] == '') { unset($voicemail_escalations[$x]); }
						}
						foreach ($voicemail_escalations as $x => $voicemail_escalation) {
							//build insert array
								$voicemail_escalation_uuid = uuid();
								$array['voicemail_escalations'][$x]['voicemail_escalation_uuid'] = $voicemail_escalation_uuid;
								$array['voicemail_escalations'][$x]['voicemail_uuid'] = $voicemail_uuid;
								$array['voicemail_escalations'][$x]['domain_uuid'] = $domain_uuid;
								$array['voicemail_escalations'][$x]['voicemail_escalation_phonenum'] = $voicemail_escalation['voicemail_escalation_phonenum'];
								$array['voicemail_escalations'][$x]['voicemail_escalation_delay'] = $voicemail_escalation['voicemail_escalation_delay'];
								$array['voicemail_escalations'][$x]['voicemail_escalation_order'] = $voicemail_escalation['voicemail_escalation_order'];
								$array['voicemail_escalations'][$x]['voicemail_escalation_description'] = $voicemail_escalation['voicemail_escalation_description'];
						}
