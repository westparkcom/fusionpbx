				...
				//add voicemail escalations
					if (permission_exists('voicemail_escalation_add') && sizeof($voicemail_escalations) > 0) {
						foreach ($voicemail_escalations as $index => $voicemail_escalation) {
							if ($voicemail_escalation['voicemail_escalation_phonenum'] == '') { unset($voicemail_escalations[$index]); }
						}
						foreach ($voicemail_escalations as $index => $voicemail_escalation) {
							//build insert array
...
//show the content
	...
	if (permission_exists('voicemail_escalation_add') || permission_exists('voicemail_escalation_edit')) {
		...
		for ($c = 0; $c < 1; $c++) {
			...
			echo "</td>\n";
			echo "<td class='vtable' align='left'>\n";
			echo "  <input class='formfld' style='width:70px' type='text' name='voicemail_escalations[".$c."][voicemail_escalation_delay]' required='required' maxlength='4' value='".$voicemail_escalation_delay."'>\n";
			echo "</td>\n";
			echo "<td class='vtable' align='left'>\n";
